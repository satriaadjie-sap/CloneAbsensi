...
export default class TableLaporanMahasiswa extends Component {
    constructor(props) {
      super(props);
      this.state = {
    ...
    GetLaporanKaryawan = () => {
      ...
      axios
      .get(`${LINK_API}Laporan/GetLaporanMahasiswa`)
      .then( async (res) => {
            this.setState({
              tableData:res.data
            })
        ...
        return(
          <DataTable.Row key={myIndex} style={styles.row}>
              ...
              <DataTable.Cell  style={{flex: 2.5}}>
                <Text style={styles.textData}>{myValue.fma_jumlah_data}</Text>
              </DataTable.Cell>
              <DataTable.Cell  style={{flex: 2.5}}>
                <Text style={styles.textData}>{myValue.fma_jumlah_kecil}</Text>
              </DataTable.Cell>
              <DataTable.Cell  style={{flex: 2.5}}>
                <Text style={styles.textData}>{myValue.fma_jumlah_sedang}</Text>
              </DataTable.Cell>
              <DataTable.Cell  style={{flex: 2}}>
                <Text style={styles.textData}>{myValue.fma_jumlah_besar}</Text>
              </DataTable.Cell>
